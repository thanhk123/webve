(function($){var DyncontEl_GoogleMapsHandler=function($scope,$){const initMap=()=>{var $map=$scope.find('.map');var map=$map[0];var bounds;let positions=$map.data('positions')||[];if(!positions.length){return}
...
if(elementSettings.style_select==='custom'){mapParams.styles=eval(elementSettings.style_map)}else if(elementSettings.style_select==='prestyle'){var fileStyle=elementSettings.snazzy_select;$.getJSON(fileStyle+".json",function(json){mapParams.styles=json})}
...
_initMap(map,mapParams);function infoWindow(marker){if(elementSettings.enable_infoWindow){if(elementSettings.infoWindow_click_to_url&&elementSettings.infoWindow_url){marker.addListener('click',function(){window.location=elementSettings.infoWindow_url.url})}else if(positions[0].infowindow){var iwOptions={content:positions[0].infowindow,}
if(elementSettings.infoWindow_panel_maxwidth.size){iwOptions.maxWidth=elementSettings.infoWindow_panel_maxwidth.size}
